...
newslog = pd.read_csv('data/newslog.csv')
...
class Article(object): 
    def __init__(self, href, pos): 
        self.href = val(href, 'href', pos, newslog)
...
class Storage: 
    def __init__(self):
        ...
        self.articles = self.construct_articles()
    ...
    def insert_new_article(self, href): 
        global newslog
        row = [ 
            href
        ]
        row_df = pd.DataFrame([row], columns=['href'])
        newslog = pd.concat([row_df, newslog], ignore_index=True)
        self.save_df(newslog, 'data/newslog.csv')
        self.articles = self.construct_articles()
